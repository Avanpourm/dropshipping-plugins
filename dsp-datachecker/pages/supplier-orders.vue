...
<!--      </Select>-->
      ...
      <div style="display: inline-block;vertical-align: middle;">
        <Input search enter-button v-model="reqData.order_numbers" style="width: 220px" placeholder="Dropshipping order Id" @on-search="searchStart"/>
      </div>
    </div>
                    ...
                    <div v-for="oitem in order.items">
                      <div class="order-item-introduce">
                        <Row type="flex">
                          ...
                          <Col span="4" v-for="itemImageUrl in oitem.image_urls"><img :src="itemImageUrl" height="90px" /></Col>
                          <Col span="16">
                            <p style="font-weight: bolder">{{oitem.product_title}} —— {{oitem.title}}</p>
                            <span v-for="oitemOption in oitem.options">{{oitemOption.name}}:{{oitemOption.value}} ; </span>
                            <br />
                  ...
                  <div slot="content">
                    <div v-for="vendor_order in order.vendor_orders">
                      ...
                      <div style="margin-bottom: 10px">
                        <Button type="primary" :loading="request_loading" @click="patchTrackingInfo(vendor_order)">回填Tracking number</Button>
                      </div>
                    </div>
                  </div>
                </Panel>
...
export default {
  ...
  methods: {
    ...
    async toOrder(order){
      ...
      // 调用 split 拆单
      await this.$axios.$post(self.requestEnvMap[self.requestEnv].product_url + '/suppliers/v2/orders/' + order.id + '/split', {}, {
        params: {},
        headers: {
          "am-api-key": self.am_api_key,
          "am-organization-id": self.reqData.organization_id
      ...
      // 调用 Place 下单接口
      await this.$axios.$post(self.requestEnvMap[self.requestEnv].product_url + '/suppliers/v2/orders/' + order.id + '/place', {}, {
        params: {},
        headers: {
          "am-api-key": self.am_api_key,
          "am-organization-id": self.reqData.organization_id
